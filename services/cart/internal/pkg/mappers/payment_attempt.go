...
func MapProtoToDomainPaymentAttempt(protoAttempt *cart.PaymentAttempt) (*models.PaymentAttempt, error) {
	if protoAttempt == nil {
		return nil, nil
	}
...
func MapDomainToProtoPaymentAttempt(attempt *models.PaymentAttempt) *cart.PaymentAttempt {
	if attempt == nil {
		return nil
	}

	return &cart.PaymentAttempt{
...
func MapPaymentAttemptsToProto(attempts []*models.PaymentAttempt) []*cart.PaymentAttempt {
	if attempts == nil {
		return nil
	}

	protoAttempts := make([]*cart.PaymentAttempt, 0, len(attempts))
